...
![Sys1](Image/new2/41.png)

以下是使用一节标称400mAh 1.2V AAAA电池测试充电时绘制的系统状况曲线（充电时间：5小时）：

![Sys1](Image/new2/42.png)

以下是使用一节标称400mAh 1.2V AAAA电池测试放电时绘制的系统状况曲线（放电时间：2小时）：

![Sys1](Image/new2/43.png)

### Emu48 39/48 Saturn模拟器

该功能可以通过运行Saturn虚拟机的方式运行HP39/48系列计算器的固件，目前仅完成了hp 39g固件的加载，退出和数据保存功能还未完成，且未进行相关优化，速度较慢。

使用方法：将HP 39g计算器的固件 rom.g39 (1MB) 或 解包后的固件 rom.g39.unpack (2MB) 放置到计算器存储空间的根目录下，在系统应用主界面点击Emu48后即可使用。对于未解包的固件载入时需要边复制进内存边解包，故载入速度较慢，请耐心等待。

退出方法：`ON + F6`

![Sys1](Image/new2/44.png)

![Sys1](Image/new2/45.png)
