...
void resetFlashRegionInfo(){
	...
	//if(_isRawFlash){
		...
		SysNandConfigBlock.iNumReservedBlocks = 0x32;
		
		
			SysNandConfigBlock.Regions[0].eDriveType = kDriveTypeSystem;
			SysNandConfigBlock.Regions[0].wTag = 0x50;
			SysNandConfigBlock.Regions[0].iNumBlks = 2;
			SysNandConfigBlock.Regions[0].iChip = 0;
			SysNandConfigBlock.Regions[0].iStartBlock = 22;
	
			SysNandConfigBlock.Regions[1].eDriveType = kDriveTypeSystem;
			SysNandConfigBlock.Regions[1].wTag = 0x60;
			SysNandConfigBlock.Regions[1].iNumBlks = 2;
			SysNandConfigBlock.Regions[1].iChip = 0;
			SysNandConfigBlock.Regions[1].iStartBlock = 24;
		
			SysNandConfigBlock.Regions[2].eDriveType = kDriveTypeSystem;
			SysNandConfigBlock.Regions[2].wTag = 0x70;
			SysNandConfigBlock.Regions[2].iNumBlks = 2;
			SysNandConfigBlock.Regions[2].iChip = 0;
			SysNandConfigBlock.Regions[2].iStartBlock = 26;
		...
		xEraseFlashBlocks(LDLBBlockPos ,1,5000);
		xEraseFlashBlocks(LDLBBlockPos+4 ,1,5000);
		
		memset(buffer,0xFF,2048);
		
		memcpy(buffer,LDLB_S01,sizeof(LDLB_S01));
		xWriteFlashPages(LDLBBlockPos * 64,  1, buffer, BCB_meta, 5000);	
		xWriteFlashPages( ( LDLBBlockPos + 4) * 64,  1, buffer, BCB_meta, 5000);	


		memcpy(buffer,&SysNandConfigBlock, 140 );//sizeof(SysNandConfigBlock));
		xWriteFlashPages(LDLBBlockPos * 64 + 1,  1, buffer, BCB_meta, 5000);
		xWriteFlashPages( ( LDLBBlockPos + 4) * 64 + 1,  1, buffer, BCB_meta, 5000);
		
		
		
		
		xEraseFlashBlocks(0 ,1,5000);
		xEraseFlashBlocks(4 ,1,5000);
		memset(buffer,0x00,2048);
		memcpy(buffer, &NCB_s00, sizeof(NCB_s00));
		xWriteFlashPages(0,1,buffer,BCB_meta,5000);
		xWriteFlashPages(256,1,buffer,BCB_meta,5000);
		...
		xEraseFlashBlocks(19 ,1,5000);
		memset(buffer,0x00,2048);
		memcpy(buffer, &DBBT, sizeof(DBBT));
		xWriteFlashPages(1216,1,buffer,ZERO_meta,5000);
		
		memset(buffer,0xFF,2048);
		memcpy(buffer, &BBCR, sizeof(BBCR));
		xWriteFlashPages(1224,1,buffer,ZERO_meta,5000);		
...
void installBootimgInPage(unsigned int page, void *buffer){
	...
	vTaskDelay(10);
	xWriteFlashPages(24 * 64 + page,1, buffer, STMP_meta, 5000);
	vTaskDelay(10);
	xWriteFlashPages(26 * 64 + page,1, buffer, STMP_meta, 5000);
	vTaskDelay(10);
