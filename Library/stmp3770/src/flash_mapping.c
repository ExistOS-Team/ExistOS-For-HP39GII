		   ...
		   (__nandConfigBlockInfo->iVersionNum  == NAND_CONFIG_BLOCK_VERSION)){
			...
			for(int i=0; i<min(nandConfigBlockInfo.iNumRegions, SUPPOSE_MAX_REGIONS); i++){
				memcpy(&RegionsInfo[i],
						...
						+sizeof(NandConfigBlockInfo_t)/4
						+i*(sizeof(NandConfigBlockRegionInfo_t)/4),
	...
	for(int pageStart =0; pageStart < 0xFFFF ; pageStart++)
		{
			...
			if( (*((unsigned int *)(&__DMA_NAND_PALLOAD_BUFFER)) == kNandMapHeaderSignature))cdcd=1;
			if( (*((unsigned int *)(&__DMA_NAND_PALLOAD_BUFFER)) == 0x68656164))cdcd=1;
			if( (*((unsigned int *)(&__DMA_NAND_PALLOAD_BUFFER)) == 0x48454144))cdcd=1;
					
					
					
			if(metadata_get_signature(metadata) == LBA_STRING_PAGE1)cdcd=1;
			if(metadata_get_signature(metadata) == PHYS_STRING_PAGE1)cdcd=1;
